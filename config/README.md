...
| <a name="input_complex_config_rules"></a> [complex\_config\_rules](#input\_complex\_config\_rules) | A range of more complex Config rules you wish to have applied. They usually carry input parameters. | `map(object({ owner = string, input_parameters = any }))` | <pre>{<br>  "CLOUDWATCH_ALARM_ACTION_CHECK": {<br>    "alarmActionRequired": "true",<br>    "insufficientDataActionRequired": "false",<br>    "okActionRequired": "false"<br>  }<br>}</pre> | no |
